...
/* DMA32 is an explicit conservative policy, valid with or without xHCI AC64.
 * Kernel/CPU pointers remain native. Page alignment is not the heap's payload
 * alignment. Disable Slot must complete before any owned DMA page is released. */
struct dma_allocation {uintptr_t p;size_t pages;int slot;};
static struct dma_allocation dma_pages[2048];
static size_t dma_count;
static void *dma_alloc(size_t bytes,int slot){
    if(!bytes||bytes>SIZE_MAX-4095||dma_count==2048)return NULL;
    size_t pages=(bytes+4095)/4096;uintptr_t p=pages_allocate_limit(pages,UINT64_C(0x100000000));
    if(!p)return NULL;
    dma_pages[dma_count++]=(struct dma_allocation){p,pages,slot};return (void*)p;
}
u64 usb_dma_bytes(void){u64 total=0;for(size_t i=0;i<dma_count;i++)total+=dma_pages[i].pages*4096;return total;}
static void dma_release_slot(int slot){
    for(size_t i=0;i<dma_count;)if(dma_pages[i].slot==slot){pages_release(dma_pages[i].p,dma_pages[i].pages);dma_pages[i]=dma_pages[--dma_count];}else i++;
}
static u32 dma_address(const volatile void *p){uintptr_t a=(uintptr_t)p;if(a>UINT32_MAX)panic("USB pointer outside DMA32 policy");return (u32)a;}
#define PA(p) dma_address(p)
static u64 control_trbs[3];
static int control_slot;
static u64 command_trb;
    ...
    for (int guard = 0; guard < 256; guard++) {
        ...
        if (type == 37) {
            ...
            klog("usb: HC event code %u (ring-full total %u)",
                 hc, ring_full_hits);
        } else if (type == EV_CMDCOMP && (((u64)t[1]<<32)|t[0])==command_trb) {
...
hid_packet_done:
            ...
            u32 port = (t[0] >> 24) & 0xFF; /* Port Status Change parameter, not Slot ID */
            if (port >= 1 && port <= (u32)max_ports) {
                volatile u32 *ps =
                    (volatile u32 *)(op + 0x400 + 0x10 * (port - 1));
                u32 v = ps[0];
                ps[0] = (v & ((1u<<9)|(3u<<14)|(7u<<25))) | (1u << 17) | (1u << 18) | (1u << 20) |
                        (1u << 21) | (1u << 22) | (1u << 23);
                ...
                if((v&(1u<<17))&&port_slot[port])reconnect_ports|=1u<<port;
                if (port < 32) pending_portc |= 1u << port; /* includes disconnect */
            }
        }
                   ...
                   int want_slot)
{
    ...
    *(volatile u32 *)db = 0;             /* command ring doorbell */
    ...
    cc_want_slot = 0;
    command_trb=0;
    if (rc < 0) {command_failed=1;klog("usb: command timeout; DMA retained, further commands disabled");}          /* timed out: un-wedge if halted */
    return rc;
}
...
static void disable_slot(int slot)
{
    if (slot >= 1 && slot <= MAX_SLOTS) {
        devs[slot].used=0;
        for(int child=0;child<8;child++){int c=devs[slot].child_slot[child];if(c&&c<=MAX_SLOTS&&devs[c].used)disable_slot(c);devs[slot].child_slot[child]=0;}
        int result=run_cmd(0, 0, 0, (u32)(TRB_DISABLESLOT << 10) | ((u32)slot << 24),500,slot);
        if(result==1){dma_release_slot(slot);dcbaa[slot]=0;}
        else klog("usb: slot %d disable failed; DMA retained, not freed",slot);
        u8 released[6]={0};
        for(int j=0;j<devs[slot].nhid;j++){
            struct hid_ep *h=&devs[slot].hid[j];
            if(h->active&&h->kind==1)kbd_inject_hid(0,released,h->prev_keys,&h->prev_mod);
            if(h->active&&h->kind==2)mouse_inject(0,0,0,0);
        }
        devs[slot].used = 0;
    }
}
...
{
    struct xdev *d = &devs[slot];
    if(d->control_failed||len>4096||(len&&!buf))return -1;
    if(len&&in)memset(d->control,0,len);
    if(len&&!in)memcpy(d->control,buf,len);
    control_slot=slot;control_trbs[1]=0;
    volatile u32 *setup = ep0_next(d);control_trbs[0]=PA(setup);
    ...
    while (now_ms() - t0 < to_ms) {
        ...
        if (cc_slot == (0x10100u | (u32)slot)){
            control_slot=0;
            if(len&&in&&(cc_code==1||cc_code==13))memcpy(buf,d->control,len);
            return (int)cc_code;
        }
        cpu_hlt();
    }
    control_slot=0;d->control_failed=1;
...
static void port_reset(int port)
{
    ...
    u64 t0 = now_ms();
    while (now_ms() - t0 < 400 && !(ps[0] & (1u << 21)) && (ps[0] & ((1u<<4)|2)) != 2) {
...
static struct xdev *slot_alloc(int slot)
{
    ...
    d->ep0 = (volatile u32 *)dma_alloc(4096,slot);
    d->control=dma_alloc(4096,slot);d->control_failed=0;
    devctx[slot] = dma_alloc(csz * 4,slot);
    inctx[slot] = dma_alloc(csz * 34,slot);
    if (!d->ep0 || !d->control || !devctx[slot] || !inctx[slot]) return NULL;
 ...
 *  - retain the route for every speed (as Linux does); clearing USB2
 *    routes aliases a hub child to its already-addressed parent
...
static int address_slot(struct xdev *d, u32 mps0, int bsr)
{
    ...
    u32 *slw = (u32 *)(ic + csz);
    /* dw0: Route String [19:0], Speed [23:20], MTT [25],
                            ...
                            int tier, struct xdev *phub, int hport)
{
    ...
    /* Enable Slot proves the controller has released any prior use of this ID. */
    dma_release_slot(slot);
    ...
    for (int j = 0; j < ncand; j++) {
        ...
        h->inr = (volatile u32 *)dma_alloc(4096,slot);
        ...
        u8 *pool = dma_alloc(IN_TRBS * IN_BUF_BYTES,slot);
...
void usb_poll_events(void){if(have_xhci)proc_events();}
...
void usb_poll(void)
{
    ...
    if (pending_portc) {                    /* real hotplug, post-boot only */
        ...
        for (int p = 1; p <= max_ports && p < 32; p++)
            if (m & (1u << p)) {
                if(reconnect_ports&(1u<<p)){int old=port_slot[p];if(old&&old<=MAX_SLOTS&&devs[old].used)disable_slot(old);port_slot[p]=0;reconnect_ports&=~(1u<<p);}
                enumerate_port(p);
...
void usb_init(void)
{
    ...
    u64 bar,bar_bytes;
    if(!pci_memory_bar(bus[0],dev[0],fn[0],0,&bar,&bar_bytes)||bar_bytes<0x1000||bar_bytes>16*1024*1024){strcpy(status_line,"usb: invalid xHCI BAR");klog("%s",status_line);return;}
    cap=mmio_map(bar,(size_t)bar_bytes);
    if(!cap){strcpy(status_line,"usb: unsafe xHCI MMIO range");klog("%s",status_line);return;}
    pci_write16(bus[0],dev[0],fn[0],4,pci_read16(bus[0],dev[0],fn[0],4)|6);
    u32 caplen = *(volatile u32 *)cap & 0xFF;
    u32 hcs1 = *(volatile u32 *)(cap + 4);
    u32 hcs2 = *(volatile u32 *)(cap + 8);
    u32 maxsp = ((hcs2 >> 27) & 31) | (((hcs2 >> 21)&31)<<5);      /* Max Scratchpad Buffers */
    u32 hcc1 = *(volatile u32 *)(cap + 0x10);
    u32 dboff = *(volatile u32 *)(cap + 0x14) & ~0x3u;
    u32 rtsoff = *(volatile u32 *)(cap + 0x18) & ~0x1Fu;
    if(caplen<0x20||caplen+0x800>bar_bytes||dboff>bar_bytes-4*(MAX_SLOTS+1)||rtsoff>bar_bytes-0x40){strcpy(status_line,"usb: xHCI register offsets out of BAR");return;}
    csz = (hcc1 & 4) ? 64 : 32;
    ...
    db = cap + dboff;
    rt = cap + rtsoff;
    klog("usb: xHCI BAR %x:%x ports %d ctx %d DMA32", (u32)(bar>>32),(u32)bar,max_ports,csz);
     ...
     * generating SMIs and can hold the controller hostage. */
    {
        ...
        int guard = 0;
        while(xecp){
            if(xecp<8||(u64)xecp*4+8>bar_bytes||guard++>=64){strcpy(status_line,"usb: invalid extended capabilities");return;}
            ...
            if (id == 1) {                    /* USB legacy support */
                ...
                if (ec[0] & (1u << 16)) {
                    ...
                    fail_flag = 1;strcpy(status_line,"usb: firmware ownership timeout");return;
                } else {
    ...
    if(!(sts[0]&1)){strcpy(status_line,"usb: controller did not halt");return;}
    if(!(*(volatile u32*)(op+8)&1)){strcpy(status_line,"usb: 4-KiB pages unsupported");return;}
    cmd[0] |= 1u << 1;                       /* HCRST */
    u64 t0 = now_ms();
    ...
    t0=now_ms();while((sts[0]&(1u<<11))&&now_ms()-t0<500)cpu_hlt();
    if(sts[0]&(1u<<11)){strcpy(status_line,"usb: controller not ready");return;}
    int slots = (int)(hcs1 & 0xFF);
    if (slots > MAX_SLOTS) slots = MAX_SLOTS;
    ((volatile u32 *)(op + 0x38))[0] = (u32)slots;

    dcbaa = (volatile u64 *)dma_alloc(4096,0);
    if(!dcbaa)goto allocation_failed;
    ...
    if (maxsp) {
        ...
        u64 *spa = (u64 *)dma_alloc(maxsp*sizeof(u64),0);
        if(!spa)goto allocation_failed;
        {
            for (u32 i = 0; i < maxsp; i++) {
                void *sb = dma_alloc(4096,0);
                if(!sb)goto allocation_failed;
                memset(sb,0,4096);spa[i] = (u64)PA(sb);
            }
            dcbaa[0] = (u64)PA(spa);
    ...
    cmd_ring = (volatile u32 *)dma_alloc(4096,0);
    if(!cmd_ring)goto allocation_failed;
    memset((void *)cmd_ring, 0, 4096);
    cmd_idx = 0; cmd_cycle = 1;
    wr64((volatile u32 *)(op + 0x18), (u64)PA(cmd_ring) | 1);

    evt_ring = (volatile u32 *)dma_alloc(EVT_TRBS * 16,0);
    if(!evt_ring)goto allocation_failed;
    ...
    u32 *erst = dma_alloc(4096,0);
    if(!erst)goto allocation_failed;
    memset(erst, 0, 4096);
    erst[0] = PA(evt_ring);
    erst[1] = 0;
    erst[2] = EVT_TRBS;
    ...
    pending_portc=reconnect_ports=0; /* discard our own boot reset changes */
    char live[128];usb_status(live,sizeof(live));
    strcpy(status_line,live);
    klog("%s", status_line);
    return;
allocation_failed:
    /* The controller is still halted: these pages were never handed to DMA. */
    dma_release_slot(0);strcpy(status_line,"usb: insufficient DMA32 memory; PS/2 fallback");klog("%s",status_line);
}

void usb_status(char *out, int max)
{
    if(!out||max<=0)return;
    char live[128],number[12];
    const char *message=status_line[0]?status_line:"usb: not probed";
    if(have_xhci){
        u32 keyboards=0,mice=0,hubs=0;
        for(int sl=1;sl<=MAX_SLOTS;sl++)if(devs[sl].used){keyboards+=!!(devs[sl].kind&1);mice+=!!(devs[sl].kind&2);hubs+=!!devs[sl].is_hub;}
        strcpy(live,"usb: xHCI live - ");fmt_u32(number,keyboards);strcat(live,number);strcat(live," keyboard, ");
        fmt_u32(number,mice);strcat(live,number);strcat(live," mouse");
        if(hubs){strcat(live,", ");fmt_u32(number,hubs);strcat(live,number);strcat(live," hub(s)");}
        if(command_failed)strcat(live,"; commands disabled after timeout");
        else if(fail_flag)strcat(live,"; see klog");
        message=live;
    }
    strncpy(out,message,(size_t)max-1);out[max-1]=0;
}
