...
static void handle_packet(void)
{
    ...
    /* PS/2 Z is positive down; SCos/HID wheel events are positive up.
     * Keep the physically accepted USB injection convention unchanged. */
...
void mouse_init(void)
{
    ...
    for(unsigned flush=0;flush<256&&(inb(MOUSE_STAT)&1);flush++)inb(MOUSE_DATA);   /* BAT + id bytes */
    mouse_write(0xF6);
    mouse_read();
    ...
    for(unsigned flush=0;flush<256&&(inb(MOUSE_STAT)&1);flush++)inb(MOUSE_DATA);   /* flush probe leftovers */
