...
static void tty_exec(char *cmd)
{
    ...
    if (!strcmp(args[0], "help")) {
        ...
        tty_print(m);
        strcpy(m, "Managed Memory: ");
        fmt_u64(n, mm_total_kb() / 1024);
        strcat(m, n); strcat(m, " MB");
        tty_print(m);
        strcpy(m, "Free Memory: ");
        fmt_u64(n, mm_free_kb() / 1024);
        strcat(m, n); strcat(m, " MB");
        tty_print(m);
        strcpy(m, "CPU Cores: ");
        ...
        if (cpu[0]) {
            ...
            strcpy(m, "Speed: ");
            fmt_u32(n, cpu_mhz());
            strcat(m, n); strcat(m, " MHz (TSC reference)");
            tty_print(m);
        }
        char ul[96];
        input_status(ul, sizeof ul);
        tty_print(ul);
        ...
        tty_print("user@scos");
        tty_print("---------------------");
        tty_print("OS:      SCos x64 development (build " SCOS_BUILD_TAG ")");
        ...
        struct app *app=app_find(args[1]);
        if(!app||app->id[0]=='_'){tty_print("appstrt: unknown app");return;}
        char file_arg[256];void *arg=NULL;
        if(nargs>2){tty_resolve(args[2],file_arg,sizeof file_arg);arg=file_arg;}
        struct window *nw = wm_open_app(args[1], arg);
        if (!nw) { tty_print("appstrt: startup failed (check memory)"); return; }
        ...
        u64 tot = 0, fre = 0; u32 a = 0, f = 0;
        mm_stats(&tot, &fre);
        mm_ops(&a, &f);
        char m[96], n[16];
        strcpy(m, "memory: total ");
        fmt_u64(n, tot); strcat(m, n); strcat(m, " KB, free ");
        fmt_u64(n, fre); strcat(m, n); strcat(m, " KB, used ");
        fmt_u64(n, tot - fre); strcat(m, n); strcat(m, " KB");
        tty_print(m);
        strcpy(m, "allocator: ");
        ...
        if (!strcmp(args[1], "panic")) {
            ...
            for (int j = 2; j < nargs; j++) {
                if (j > 2) strncat(msg," ",sizeof(msg)-strlen(msg)-1);
                strncat(msg,args[j],sizeof(msg)-strlen(msg)-1);
            }
        ...
        tty_print("SCos unnumbered x64 development (build " SCOS_BUILD_TAG
                  ") - kernel console v2.0");
    ...
    while (!tty_exit) {
        ...
        while (kbd_poll(&ke)) {
            ...
            if (ke.keycode == '\n') {
                ...
                if(tty_exit)break; /* leave subsequent input for the new foreground */
                tty_draw();
            } else {
        ...
        if (!got) {wm_in_idle=1;cpu_hlt();wm_in_idle=0;}
    }
}
