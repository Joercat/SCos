...
$(BUILD)/vectors.o: $(BUILD)/vectors.S
	$(CC) -m64 -ffreestanding -fno-pie -c $< -o $@
$(BUILD)/kernel.elf: $(BUILD)/entry.o $(OBJECTS) $(BUILD)/vectors.o kernel/x86_64/linker.ld Makefile
	$(LD) -m elf_x86_64 -pie --no-dynamic-linker -Bsymbolic --build-id=none -z noexecstack -z max-page-size=0x1000 -T kernel/x86_64/linker.ld -o $@ $(BUILD)/entry.o $(OBJECTS) $(BUILD)/vectors.o
