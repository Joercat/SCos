...
static SystemSettings g_settings = {.theme_id = 2, // Matrix Green theme
                                    .brightness = 80,
                                    .sound_enabled = 1,
                                    .boot_timeout = 5,
                                    .text_size = 1,
...
static const Theme themes[] = {
    ...
    {"Terminal Green", COLOR_BLACK, COLOR_GREEN, COLOR_LIGHT_GREEN,
     COLOR_YELLOW},
    ...
    {"Purple Haze", COLOR_MAGENTA, COLOR_WHITE, COLOR_LIGHT_MAGENTA,
     COLOR_YELLOW}};
...
static void draw_horizontal_line(int y, int start_x, int end_x, char ch,
                                 uint8_t color) {
...
static void draw_progress_bar(int x, int y, int width, int value, int max_value,
                              uint8_t color) {
...
static uint8_t get_accent_color() {
  return themes[g_settings.theme_id].accent_color;
}
static uint8_t get_selected_color() {
  return themes[g_settings.theme_id].selected_color;
}
...
static void draw_footer() {
  ...
  center_text(23, "Arrow Keys: Navigate | Enter: Select | ESC: Back/Exit",
              footer_color);
}

static void draw_main_menu() {
  const char *menu_items[] = {"Display Settings",  "System Settings",
                              "Audio Settings",    "Network Settings",
                              "Security Settings", "About System"};

  int num_items = sizeof(menu_items) / sizeof(menu_items[0]);
  uint8_t normal_color = MAKE_COLOR(get_fg_color(), get_bg_color());
  uint8_t selected_bg_color =
      MAKE_COLOR(get_selected_color(), get_accent_color());

  vga_put_string(5, 5,
                 "Main Menu:", MAKE_COLOR(get_accent_color(), get_bg_color()));
...
static void draw_display_settings() {
  ...
  // Brightness setting
  uint8_t bright_color =
      (selected_item == 1) ? selected_bg_color : normal_color;
  ...
  uint8_t cursor_color =
      (selected_item == 3) ? selected_bg_color : normal_color;
  vga_put_string(10, 13, "Cursor Blink:", cursor_color);
  vga_put_string(25, 13, g_settings.cursor_blink ? "Enabled" : "Disabled",
                 cursor_color);
  if (selected_item == 3) {
  ...
  vga_put_string(12, 17, "Sample text in current theme",
                 MAKE_COLOR(get_fg_color(), get_bg_color()));
  vga_put_string(12, 18, "Highlighted text example",
                 MAKE_COLOR(get_selected_color(), get_accent_color()));
}
...
static void draw_system_settings() {
  ...
  vga_put_string(25, 9, g_settings.sound_enabled ? "Enabled" : "Disabled",
                 sound_color);
  if (selected_item == 1) {
  ...
  // Actions
  uint8_t restart_color =
      (selected_item == 2) ? selected_bg_color : normal_color;
...
void settings_navigate_down() {
  ...
  if (current_menu == 1)
    max_items = 4; // Display settings
  if (current_menu == 2)
    max_items = 4; // System settings
...
void settings_back() {
  if (current_menu == 0) {
    ...
    center_text(12, "Returning to desktop...",
...
void openSettingsSimple() {
  ...
  center_text(14, "Use openSettings() for full interface",
              MAKE_COLOR(get_accent_color(), get_bg_color()));
}
#include <stdint.h>
...
Desktop::Desktop(int width, int height)
    : width_(width), height_(height), numWindows_(0), capacity_(5) {
...
void Desktop::handleInput(int key) {
  ...
  // You can implement key handling logic here, such as moving focus between
  // windows
}
// window_manager.hpp
...
WindowManager::~WindowManager() { delete desktop_; }

Window *WindowManager::createWindow(int x, int y, int width, int height,
                                    const char *title) {
...
void WindowManager::draw() { desktop_->draw(); }

void WindowManager::handleInput(int key) { desktop_->handleInput(key); }
// terminal.hpp
#ifndef TERMINAL_HPP
#define TERMINAL_HPP
...
void Settings::openSettings() {
  // Implement settings display logic here - no std::cout in freestanding
  // environment
}

void Settings::handleInput(char key) {
  // Implement settings input handling logic here - no std::cout in freestanding
  // environment
}
// about.hpp
#ifndef ABOUT_HPP
...
void About::openAbout() {
  // Implement about display logic here - no std::cout in freestanding
  // environment
}
// main.cpp
#include "about.hpp"
