...
static int owned_layout(const u8 *p)
{
    if (memcmp(p + 400, "SCOSDISK32v1", 12) || p[510] != 0x55 || p[511] != 0xaa) return 0;
    if (*(const u32 *)(p+412) != 2048 || *(const u32 *)(p+416) != 256 ||
        *(const u32 *)(p+420) != 16384 || !*(const u32 *)(p+424) ||
        *(const u32 *)(p+424)>0x60000) return 0;
    for (int i=446;i<510;i++) if (p[i]) return 0; /* no partitioned disks */
    return 1;
}
int fs_image_available(void) { return storage_dev >= 0; }
u32 fs_image_kernel_bytes(void) { return storage_dev >= 0 ? *(u32 *)(storage_mbr+424) : 0; }
...
{
    if (storage_dev < 0 || lba >= 16384 || count > 16384-lba) return 0;
...
int fs_image_save(void)
{
    ...
    u8 verify[512];
    if (!ata_read_sectors(0,1,verify) || memcmp(verify,storage_mbr,512))
        return image_error("disk ownership changed; no write attempted");
    if (!img_buf) img_buf=palloc(IMAGE_BYTES);
...
static int image_apply(const u8 *p, u32 len)
{
    ...
    struct vfs_node *fresh=palloc(sizeof(*fresh));
    if (!fresh) return 0;
